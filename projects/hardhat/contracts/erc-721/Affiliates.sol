...
contract Affiliates is ERC721, Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(address => uint256)) parentChildTokenId;
    mapping(uint256 => mapping(uint256 => address)) tokenIdPriceOfferer;
    ...
    function setFounderAddress(address fdrAddress) public onlyOwner {
        ...
        uint256 totalSupply = IERC20(fdrAddress).totalSupply();
        require(totalSupply != 0, "Provided Founder ERC20 contract should not have a total supply of 0");
    ...
    // sends a payment to the affiliate and split it among its parent.
    // if there is no 7 levels, send the remaining to the founders contract:
    function sendPayment(address child, uint256 amount) public nonReentrant {
        require(amount > 0, "amount must be greater than zero");

        // transfer the payment to the contract:
        _transferCurrencyOnBehalf(msg.sender, address(this), amount);

        // receive payment:
        _sendPayment(child, amount, 0, 0);
    }

    function claimPayment(address sendTo, uint256 amount) public nonReentrant {
        require(amount > 0, "amount must be greater than zero");
        ...
        _transferCurrencyFromContract(sendTo, transferAmount);
    ...
    function registerOffer(uint256 tokenId, uint256 price) public {
        ...
        _transferCurrencyOnBehalf(msg.sender, address(this), price);
        ...
        tokenIdPriceOfferer[tokenId][price] = msg.sender;
    ...
    function withdrawOffer(uint256 tokenId) public nonReentrant {
        ...
        _transferCurrencyFromContract(msg.sender, offerPrice);
        
        _deleteOffer(msg.sender, tokenId, offerPrice);

        // emit:
        emit WithdrawOffer(msg.sender, tokenId);
    }
    ...
    function acceptOffer(address sendTo, uint256 tokenId) public nonReentrant {
        address owner = _ownerOf(tokenId);
        ...
        // fetch the offerer:
        address offerer = tokenIdPriceOfferer[tokenId][offerPrice];

        // send the currency to the offer accepter:
        _transferCurrencyFromContract(msg.sender, offerPrice);

        // send the token to the offererer:
        safeTransferFrom(owner, offerer, tokenId);

        // delete the offer:
        _deleteOffer(offerer, tokenId, offerPrice);

        // emit:
        emit AcceptOffer(offerer, sendTo, tokenId, offerPrice);
    }
    ...
    function getTokenId(address parent, address child) public view returns(uint256) {
        return parentChildTokenId[parent][child];
    }

    function _transferCurrencyOnBehalf(address from, address to, uint256 amount) private {
    ...
    function _sendPayment(address child, uint256 amount, uint256 level, uint256 totalPaid) private {
        require(founderAddress != address(0), "Founder Address not set");
        require(amount > 0, "amount must be greater than zero");
        ...
        // emit:
        emit PaymentReceived(parent, share, amount);

        // continue next level:
        uint nextLevel = level + 1;
        _sendPayment(parent, amount, nextLevel, totalPaid + share);
    }
    ...
    function _deleteOffer(address from, uint256 tokenId, uint256 offerPrice) private {
        ...
        delete tokenIdPriceOfferer[tokenId][offerPrice];
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory data) public override {
        ...
        for (uint256 i = 0; i < childList.length; i++) {
            ...
            delete parentChildTokenId[from][child];
            parentChildTokenId[to][child] = tokenId;
        }
